...
export class ModifierManager implements IModifierManager {
  ...
  remove(control: string, modifiers: IModifier[]): IModifier[] | undefined {
    const searchedModifier = modifiers.find(
      (modifier) => modifier.name === control,
    );
    if (searchedModifier) {
      const searchIndex = modifiers.indexOf(searchedModifier);
      return modifiers
        .slice(0, searchIndex - 1)
        .concat(modifiers.slice(searchIndex + 1));
    }
  }
  getActive(
    control: string,
    ...
    if (modifiers?.includes(modifier)) {
      return this.remove(control, modifiers);
    } else {
      return this.add(modifier, modifiers);
    }
  }
}
